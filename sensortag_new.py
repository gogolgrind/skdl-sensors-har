...
def main():
    ...
    name = raw_input()
    newpath = ((r'/home/pi/Desktop/Folder/'))
    ...
    parser.add_argument('host1', action='store',help='MAC of BT device')
    parser.add_argument('host2', action='store',help='MAC of BT device')
    
    #parser.add_argument('-n', action='store', dest='count', default=0,
            #type=int, help="Number of times to loop data")
    parser.add_argument('-t',action='store',type=float, default=0.01, help='time between polling')
    ...
    arg1 = parser.parse_args(sys.argv[2:])
    arg2 = parser.parse_args(sys.argv[2:])

    print('Connecting to ' + arg.host)
    print('Connecting to ' + arg1.host1)
    print('Connecting to ' + arg2.host2)
    tag = SensorTag(arg.host)
    tag1 = SensorTag(arg1.host1)
    tag2 = SensorTag(arg2.host2)
    ...
    while True:
       ...
       writer.writerow([ti,tag.accelerometer.read(),tag.gyroscope.read(), tag.magnetometer.read(),
       tag1.accelerometer.read(),tag1.gyroscope.read(), tag1.magnetometer.read(), 
       tag2.accelerometer.read(),tag2.gyroscope.read(), tag2.magnetometer.read()])
       #if (arg.light or arg.all) and tag.lightmeter is not None:
